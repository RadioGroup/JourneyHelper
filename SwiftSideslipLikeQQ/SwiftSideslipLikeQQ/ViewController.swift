...
class ViewController: UIViewController {
    ...
    @IBOutlet weak var newcButton: UIButton!
    ...
    override func viewDidLoad() {
        ...
        userDefaults.setInteger(1, forKey: "Log")
        let logFlag = userDefaults.integerForKey("Log")
        if(logFlag==0)
        {
            let user = "Hoatson"
            let password = "root"
            
            Alamofire.request(.GET, "http://172.50.180.239/login?userName=\(user)&passWord=\(password)")
                ...
                {   response in
                    ...
                    let user = data!["user"]
                    print("数据\(status)\(user)")
                    //                userDefaults.setObject(response in, forKey: "data")
